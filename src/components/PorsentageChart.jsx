...
export function PorsentageChart() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const response = await fetch('http://127.0.0.1:5000/promedioAtencion');
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const response = await fetch('http://127.0.0.1:5000/promedioAtencionM');
        const dataResponse = await response.json();
        const newLabels = dataResponse.map(item => item.especialidad);
        const newDataValues = dataResponse.map(item => item.promedio_tiempo_atencion);
        const newBackgroundColors = newDataValues.map(() => getRandomColor());
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const response = await fetch('http://127.0.0.1:5000/promedioAtencionF');
        const dataResponse = await response.json();
        const newLabels = dataResponse.map(item => item.especialidad);
        const newDataValues = dataResponse.map(item => item.promedio_tiempo_atencion);
        const newBackgroundColors = newDataValues.map(() => getRandomColor());
